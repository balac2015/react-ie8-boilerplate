...
$.ajaxSetup({
    timeout: 15000,
    beforeSend: xhr => xhr.setRequestHeader(AUTH_TOKEN_KEY, getCookie(AUTH_TOKEN_KEY)),
})
$(document).ajaxComplete((event, xhr, options) =>
    xhr.always((data, status, xhr) => {
        switch (status) {
            case "abort":
            case "error":
            case "nocontent":
...
service.interceptors.response.use(
    response => {
        ...
        return /^2\d+|304$/.test(status) ? Promise.resolve(response) : Promise.reject(response)
    },
    error => {
...
// 权限检查
export const permission = route =>
    Promise.resolve(route).then(v => {
        const login = true
        const auth = true
        const history = { push() {} }
        if (!login) {
