...
export const isFunction = v =>
    typeof v === "function" || getType(v) === "Function" || getType(v) === "GeneratorFunction"
export const isNum = v =>
    ["number", "string"].includes(typeof v) && /^\s*(?:\+|-)?\s*\d+(?:.\d*)?\s*$/.test(v)
export const isInt = v =>
    ["number", "string"].includes(typeof v) && /^\s*(?:\+|-)?\s*\d+\s*$/.test(v)
...
export const lazyFn = fn =>
    ...
    new Promise((resolve, reject) => fn((...args) => resolve(args.length > 1 ? args : args[0])))
...
export const verClient = () => {
    ...
    let match
    match = ua.match(/msie[/\s]*([.\d]+)/i) || ua.match(/rv:([.\d]+)[)\s]+like gecko/i)
    if (match) {
    ...
    match = ua.match(/ucweb[/\s]*([.\d]+)/i) || ua.match(/ucbrowser[/\s]*([.\d]+)/i)
    if (match) {
        return Object.assign(res, {
            type: "uc",
            version: match[1],
...
export const urlArgs = (v, b) => {
    ...
    if (b) {
        ...
        String(v || "").replace(/^([^?#]*)(\?[^#]*)?(#.*)?$/, (match, main, args, hash) => {
            obj.main = decodeURIComponent(main || "")
            obj.hash = decodeURIComponent(String(hash || "").slice(1))
            String(args || "").replace(/(\?|&)([^&=]*)(=[^&]*)?/g, (match, prefix, key, value) => {
                key = decodeURIComponent(key || "")
                value = decodeURIComponent(String(value || "").slice(1))
                if (key || value) {
                    obj.args[key] = value
                }
                return ""
...
export const getArea = division => {
    ...
    const getPid = id =>
        id % 100
            ? String(id).slice(0, -2) + "00"
            : id => (id % 10000 ? String(id).slice(0, -4) + "0000" : null)
    String(division || "").replace(/(\d{6})\s+(\S+)/g, (match, code, name) => {
        const item = {
            id: code,
            pid: getPid(code),
            value: code,
